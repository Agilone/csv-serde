...
public final class CSVSerde implements SerDe {
	...
	private String getHeader(final Configuration conf, String path) throws IOException {
		FileSystem fs = FileSystem.get(conf);
		FileStatus contents[] = fs.listStatus(new Path(path));
		int i=0;
		while(contents[i].getLen() == 0L) {
			i++;
		}
		FSDataInputStream fis = fs.open(contents[i].getPath());
